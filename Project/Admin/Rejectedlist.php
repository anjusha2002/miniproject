...
if(isset($_GET["aid"]))
{
    $accept="update tbl_user set user_status='1' where user_id='".$_GET["aid"]."'";
    if($conn->query($accept))
    {
		 	header("Location:Rejectedlist.php");
	}
	//Email Code Start
    ...
    $mail->Body = "Hello " .$_GET["name"].",thank you for choosing Malanad Juice.Your registration is  rejected.Thank you ";
  if($mail->send())
  {
    echo "Sended";
  }
...
<table border="1" align="center">
 ...
 <?php
		$selQry="select * from tbl_user u inner join tbl_place p on u.place_id=p.place_id inner join tbl_district d on        
		 p.district_id=d.district_id where user_status=2";
 ...
 ?>
 		<tr>
            ...
            <td><a href="Rejectedlist.php?aid=<?php echo $data["user_id"]?>&user=<?php echo $data["user_email"]?>&name=<?php echo $data["user_name"]?>">Accept</a></td>
